        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                for i, file_key in enumerate(parquet_files, 1):
                    ...
                    logger.info(f"Memory usage for {file_key}: {table.nbytes / (1024*1024):.2f} MB")
                
                # Concatenate all tables using correct pyarrow method
                ...
                logger.info(f"Merged table size: {merged_table.nbytes / (1024*1024):.2f} MB")
                ...
                logger.info(f"Successfully completed merge process in {total_time:.2f}s")
                logger.info(f"Final merged file stats:")
                logger.info(f"- Total rows: {merged_table.num_rows}")
                logger.info(f"- Total size: {merged_table.nbytes / (1024*1024):.2f} MB")
                logger.info(f"- Average processing time per file: {total_time/len(parquet_files):.2f}s")
